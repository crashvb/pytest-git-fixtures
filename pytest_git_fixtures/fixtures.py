...
def git_repo(
    ...
    # path_upstream = tmp_path.joinpath("upstream")
    # subprocess.run(
    #     ["git", "init", "--bare", str(path_upstream)],
    #     check=True,
    #     env=environment,
    # )
    # path_fork = tmp_path.joinpath("fork")
    # subprocess.run(
    #     ["git", "init", "--bare", str(path_fork)],
    #     check=True,
    #     env=environment,
    # )
    #
    # # Clone the bare repository to populate the master branch (or it will not
    # # be possible to create a work tree) ...
    # path_clone = tmp_path.joinpath("clone")
    # subprocess.run(
    #     ["git", "clone", str(path_upstream), str(path_clone)],
    #     check=True,
    #     env=environment,
    # )
    #
    ...
    # subprocess.run(
    #     ["git", "remote", "add", "fork", str(path_fork)],
    #     check=True,
    #     cwd=str(path_clone),
    #     env=environment,
    # )
    ...
    # file_path = path_clone.joinpath("test.txt")
    # file_path.write_text("This is a test file.", encoding="utf-8")
    # subprocess.run(
    ...
    #         "git",
    #         "-c",
    #         f"gpg.program={gnupg_keypair.gnupg_home}/gpg-wrapper",
    #         "commit",
    #         "--message",
    #         "Initial commit.",
    #     ],
    #     check=True,
    #     cwd=str(path_clone),
    #     env={**environment, **{"GNUPG_PASSPHRASE": gnupg_keypair.passphrase}},
    # )
    #
    # # Replicate changes to upstream ...
    # subprocess.run(
    ...
    # path_work_tree = tmp_path.joinpath("work-tree")
    # subprocess.run(
    #     ["git", "worktree", "add", "--force", str(path_work_tree), "master"],
    #     check=True,
    #     env={**environment, **{"GIT_DIR": str(path_upstream)}},
    # )
